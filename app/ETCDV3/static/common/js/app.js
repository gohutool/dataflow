...
initAjax = function(){
	...
	$.ajax = function(url, options){
		...
		var newsuccess = function(data, textStatus, xhr){
			  ...
			  {
				  if(xhr.responseJSON && ((xhr.responseJSON.status > 0 && xhr.responseJSON.status!==true) || xhr.responseJSON.status===false) ){
...
$.app.ajax = function(urlstr, datastr, method, datatype, fn, ignoreerror, progressing, requestHeader, issync){
...
$.app.post = function(url, datastr, fn, ignoreerror, progressing, requestHeader, issync){
	$.app.ajax(url, datastr, 'POST', null, fn, ignoreerror, progressing, requestHeader, issync);
};

$.app.postJson = function(url, datastr, fn, ignoreerror, progressing, requestHeader, issync){
	if(typeof datastr == 'object'){
	    datastr = $.extends.json.tostring(datastr)
		
		// if (!requestHeader)
		// 	requestHeader = {}
		requestHeader = $.extend(requestHeader, {
			'content-type': "application/json", 
		});
		
	}
	$.app.ajax(url, datastr, 'POST', "json", fn, ignoreerror, progressing, requestHeader, issync);
};
				  ...
				  $.app.postForm(
					 ...
					 function(data) {
						if (data.status) {
							...
							$.app.localStorage.saveItem(window.app.clientId+'.token', data.token);
